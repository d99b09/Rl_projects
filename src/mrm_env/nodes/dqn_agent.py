...
MODEL_NAME = 'Random_target__fix_joint'
...
class DQNAgent:
    ...
    def create_model(self):
        ...
        if LOAD_MODEL != None:
            ...
            model = Sequential()
            model.add(Dense(48, input_shape=env.OBSERVATION_SPACE_VALUES))
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
        ...
        if not episode % AGGREGATE_STATS_EVERY or episode == 1:
            ...
            if min_reward >= MIN_REWARD:
                save_path = 'models/%(MODEL_NAME)s__%(max_reward).2fmax_%(average_reward).2favg_%(min_reward).2fmin__%(time)d.model' % {"MODEL_NAME": MODEL_NAME, "max_reward": max_reward, "average_reward":
